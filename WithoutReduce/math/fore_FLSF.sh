...
n=1
Method=""
for line in `cat FLSF/ef-math$1.txt | tr -d '\r' `
do
	if [[ $(($n%2)) == 1 ]];then
		Method=$line
	else
		...
		echo $Method ${str} >> e_FLSF/ef-math$1.txt
	fi
	n=$(($n+1))
done

for line in `cat FLSF/ep-math$1.txt | tr -d '\r' `
do
	if [[ $(($n%2)) == 1 ]];then
		Method=$line
	else
		...
		echo $Method ${str} >> e_FLSF/ep-math$1.txt
	fi
	n=$(($n+1))
done