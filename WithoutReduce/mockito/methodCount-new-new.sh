...
do 
	...
	if [[ $ef1 -ge 1 ]];then
		echo ${method%=*} `grep -c ^$method Yichuli/mockito$1-failTS.txt` >> SBFL/ef-mockito$1.txt
	...
	if [[ $ep1 -ge 1 ]];then
		echo ${method%=*} `grep -c ^$method Yichuli/mockito$1-passTS.txt` >> SBFL/ep-mockito$1.txt
