...
n=1
Method=""
for line in `cat FLSF/ef-lang$1.txt | tr -d '\r' `
do
	if [[ $(($n%2)) == 1 ]];then
		Method=$line
	else
		...
		echo $Method ${str} >> e_FLSF/ef-lang$1.txt
	fi
	n=$(($n+1))
done

for line in `cat FLSF/ep-lang$1.txt | tr -d '\r' `
do
	if [[ $(($n%2)) == 1 ]];then
		Method=$line
	else
		...
		echo $Method ${str} >> e_FLSF/ep-lang$1.txt
	fi
	n=$(($n+1))
done