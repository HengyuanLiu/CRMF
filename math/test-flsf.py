...
with open('SumOfTestsuites-math-fail.txt','r') as fileSOT:
	for lineSOT in fileSOT:
		...
		add_sheet_xls('Test4-flsf.xls', sheet_name, value_title )

		#------------------添加ef数据
		def write_Rf_append(path,value,sheet_name): 
			workbook = xlrd.open_workbook(path)
		...
		Filename = "e_FLSF/ef-" + sheet_name + ".txt"
		with open(Filename,'r') as file:
			for line in file:
				line_name = line.strip('\n').split(' ')
				# print("%s |" % line_name[1] )
				write_Rf_append("Test4-flsf.xls",line_name,sheet_name)

		#------------------添加ep数据
		def write_Rp_append(path,value,sheet_name,row): 
			workbook = xlrd.open_workbook(path)
			worksheet = workbook.sheet_by_name(sheet_name)
			new_workbook = copy(workbook)
			new_worksheet = new_workbook.get_sheet(sheet_name)
			new_worksheet.write(row, 2, float(value[1]))
			new_workbook.save(path)



		Filename = "e_FLSF/ep-" + sheet_name + ".txt"
		row = 1
		with open(Filename,'r') as file:
			for line in file:
				line_name = line.strip('\n').split(' ')
				# print(line_name[2])
				# print("%s |" % line_name[1] )
				write_Rp_append("Test4-flsf.xls",line_name,sheet_name,row)
				row = row + 1
		#此时row等于需要填的总行数
		#------------------添加Total_f数据
		...
		def write_Tarantula_flsf_append(path,sheet_name,rows): 
			workbook = xlrd.open_workbook(path)
			worksheet = workbook.sheet_by_name(sheet_name)
			new_workbook = copy(workbook)
			new_worksheet = new_workbook.get_sheet(sheet_name)
		...
		while row > 1 :
			row = row -1
			write_int_append("Test4-flsf.xls",lineSOT_arry[1],sheet_name,row,3)		
			write_int_append("Test4-flsf.xls",total_p,sheet_name,row,4)
		
		write_Tarantula_flsf_append("Test4-flsf.xls",sheet_name,row_len)
		...
		# 		line_name = line.strip('\n').split(' ')
		# 		# print("%s |" % line_name[1] )
		# 		write_int_append("Test4-flsf.xls",line_name,1,row,6)
		# 		row = row + 1
